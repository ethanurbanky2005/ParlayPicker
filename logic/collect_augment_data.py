...
def get_teams():
    ...
    if all_team_train == True:
        with open(join(getcwd(),'Data/team_rankings_year.yaml')) as file:
    ...
    if year_list:
        for year in year_list:
            ...
            if year == 2024:
                ...
                if exists(join(getcwd(),'all_data_2024.csv.csv')):
                    remove('all_data_2024.csv.csv')
                    remove('x_feature_regression_2024.csv')
                    remove('y_feature_regression_2024.csv')
                if not exists(join(getcwd(),'all_data_2024.csv.csv')):
                    final_data.to_csv(join(getcwd(),'all_data_2024.csv.csv'),index=False)
